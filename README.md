...
2. **Manually configure the exports to BigQuery for Standard usage cost, Detailed usage cost, Pricing, and Committed Usage Discounts Export in GCP** (this cannot be done via Terraform):
...
|------|-------------|------|---------|:--------:|
| <a name="input_api_exclusions"></a> [api\_exclusions](#input\_api\_exclusions) | Map of project IDs to lists of APIs to exclude from being enabled in those projects. Useful for excluding APIs that are not needed or cause errors in specific projects. Example: { "my-project-id" = ["compute.googleapis.com"] } | `map(list(string))` | `{}` | no |
